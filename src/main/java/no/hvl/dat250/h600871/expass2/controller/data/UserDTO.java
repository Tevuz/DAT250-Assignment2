...
public record UserDTO(Optional<Long> id, Optional<String> username, Optional<String> email, Optional<List<UserPollsEntries>> polls, Optional<List<UserVoteEntries>> votes) {
    ...
    public static record UserVoteEntries(long id, String question, List<String> voteOptions) {
        UserVoteEntries(Vote vote) {
            this(vote.getId(), vote.getVoteOption().getFirst().getPoll().getQuestion(), vote.getVoteOption().stream().map(VoteOption::getCaption).toList());
        }
    }
